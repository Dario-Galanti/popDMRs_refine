...
## Run: python3 DMR_distance_distr.py CpG_bedfile (NB: there needs to be CHG and CHH corresponding files with same dir structure) plotname.pdf
## Run: sbatch --partition test --cpus-per-task 2 --mem 10G --time 02:00:00 --wrap "python3 DMR_distance_distr.py merged_DMRs/CpG/DMRs_CpG_merged.bed pop_merged_DMRs_distance.pdf"
...
binsize=10
# Add "kde=False" to distplot function to remove density function and plot count instead of Density
# plot CpG
reg_bed_dist(fin_CpG)
bins=int(max(dist_list)/binsize)
sns.distplot(dist_list, color="#CD5B45", label="CG", **kwargs, bins=bins)
#plot CHG
reg_bed_dist(fin_CHG)
bins=int(max(dist_list)/binsize)
sns.distplot(dist_list, color="#EEC900", label="CHG", **kwargs, bins=bins)
#plot CHH
reg_bed_dist(fin_CHH)
bins=int(max(dist_list)/binsize)
sns.distplot(dist_list, color="#1E90FF", label="CHH", **kwargs, bins=bins)
